...
Markdown files should be placed in `vpsite/docs`, inside a folder that corresponds with what the page is describing (for example, if you are adding a PQSMod it should probably go in `vpsite/docs/Syntax/PQSMods`).
...
If you want your new page to appear in the sidebar, you will need to add an entry in the sidebar configuration in `vpsite/docs/.vitepress/config.mts`. Find the correct section (or create one if it is a new section) and add an item with the page's title and link.
...
2. Navigate to the `vpsite` directory.
