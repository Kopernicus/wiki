...
|innerRadius|Single|The lower bound of the atmosphere effect in 1/6000ths of a meter.|
|outerRadius|Single|The upper bound of the atmosphere effect in 1/6000ths of a meter.|
|invWaveLength|Color|The inverse wavelength. Either this OR `waveLength` should be used. The inverse wavelength is equivalent to each of the color values of the `waveLength` as x: `1/(x^4)` with an alpha of 0.5.|
|waveLength|Color|The wavelength of the atmosphere. Either this OR `invWaveLength` should be used. The wavelength is equal to each of the color values of the `invWaveLength` as x: `sqrt(sqrt(1/x))` with an alpha of 0.5.|
|samples|Single|?|
