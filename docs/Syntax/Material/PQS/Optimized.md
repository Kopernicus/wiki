...
Select the material by setting `materialType = Optimized` in the [`PQS { }`](/Syntax/PQS) node. The `Material { }` node then accepts the properties listed below.
...
* This variant normal-maps **only the mid band**. If the low or high bands need surface relief of their own, use the full [Main](/Syntax/Material/PQS/Main) or [Extra](/Syntax/Material/PQS/Extra) shader, which give every band an independent normal map.
