...
`Extra` is, in effect, the full [Main](/Syntax/Material/PQS/Main) shader — every elevation band carries its own normal map with independent near/far tiling — with one addition: it exposes `oceanFogDistance`, so you can tune how quickly the surface fades into the underwater fog colour. It is one of the pre-1.8 terrain shaders. If you do not need the ocean-fog control, [Main](/Syntax/Material/PQS/Main) is identical in every other respect; for a lighter normal-map budget, see [Optimized](/Syntax/Material/PQS/Optimized).
...
Select the material by setting `materialType = Extra` in the [`PQS { }`](/Syntax/PQS) node. The `Material { }` node then accepts the properties listed below.
...
7. **Aerial-perspective & ocean fog** — on a body with an atmosphere, a distance haze is mixed in last. Its colour is read from `fogColorRamp` indexed by the sun angle, and its strength grows with distance and `globalDensity`. A second, underwater **ocean fog** term reads the same ramp at a different row and fades over the `oceanFogDistance` — the control this variant adds over the plain `Main` shader. On an airless body the whole fog stage is compiled out and these properties do nothing.
...
* `Extra` differs from the plain [Main](/Syntax/Material/PQS/Main) shader only by exposing `oceanFogDistance`; everything else — the three normal-mapped bands, the near/far tiling, the steep overlay — is the same.
...
* `materialType = Extra` is also accepted under its older alias `AtmosphericExtra`, which you may see in existing configs; the two select the same shader.
